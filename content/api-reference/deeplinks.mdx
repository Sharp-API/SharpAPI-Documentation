...
<Callout type="info">
The `id` parameter accepts both **odds IDs** (from `/odds`, `/odds/best`) and **opportunity hash IDs** (from [+EV](/api-reference/opportunities-ev), [Arbitrage](/api-reference/opportunities-arbitrage), [Middles](/api-reference/opportunities-middles), and [Low Hold](/api-reference/opportunities-low-hold) endpoints). Odds IDs resolve to market-specific links; opportunity hash IDs resolve to event-level links.
</Callout>
