...
| `sportsbook` | string | No | Sportsbook ID filter (defaults to all available for your tier) |
| `market` | string | No | Market type filter (defaults to all) |
...
| `meta.missing_events` | string[] | Event IDs that were not found or had no odds |
| `meta.max_batch` | integer | Maximum batch size for your tier |
| `meta.filters` | object | Applied filters (`sportsbook`, `market`) |

<Callout type="info">
If an event ID in your request has no odds (e.g., the event has ended or doesn't exist), it will appear in `missing_events` rather than causing an error. The response will still include odds for all valid events.
</Callout>

## Response Schema

Each item in the `data` array is an event object with nested odds:
...
async function refreshWatchlist(eventIds) {
  ...
  if (meta.missing_events.length > 0) {
    console.warn('Missing events:', meta.missing_events);
  }
