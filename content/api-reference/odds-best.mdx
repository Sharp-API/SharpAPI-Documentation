---
description: "GET /api/v1/odds/best — Get the best available odds for each selection across all sportsbooks with consensus odds and market hold calculations."
---
...
data.forEach(item => {
  console.log(
    ...
    ` | Consensus: ${item.consensus_odds.american} | Hold: ${item.market_hold.toFixed(1)}%`
  );
});
```
  </Tabs.Tab>
