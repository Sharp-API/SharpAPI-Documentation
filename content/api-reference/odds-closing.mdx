...
Each sportsbook is captured independently the first time it transitions a given event to `is_live=true`. Captured snapshots live in Valkey for **48 hours** after the event starts. Beyond that window, use [`/api/v1/historical/odds/closing`](/api-reference/enterprise) (Enterprise, ClickHouse-backed).
...
| 401 | `missing_api_key` / `invalid_api_key` | See [Authentication](/authentication). |
...
- [`/api/v1/historical/odds/closing`](/api-reference/enterprise) — Enterprise; long-term ClickHouse history beyond the 48h window.
- [`/api/v1/historical/clv`](/api-reference/enterprise) — CLV analytics aggregated across opportunities.
