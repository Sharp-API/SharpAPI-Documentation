...
| `market` | string | all | Filter by market type (e.g., `moneyline`, `spread`). Supports category aliases — see [Odds: Market Category Aliases](/api-reference/odds#market-category-aliases). |
...
| `odds_changed_at` | string | When this book's price, line, or `is_live` flag last actually changed. On Pinnacle, carries forward across unchanged refreshes — see [Understanding Pinnacle's `odds_changed_at`](/concepts/pinnacle-odds-changed-at). |
