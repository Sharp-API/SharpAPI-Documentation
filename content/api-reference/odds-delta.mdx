...
| `market` | string | all | Filter by market type (e.g., `moneyline`, `spread`, `total`). Supports category aliases — see [Odds: Market Category Aliases](/api-reference/odds#market-category-aliases). |
...
```json
{
  "error": {
    ...
    "docs": "https://docs.sharpapi.io/api-reference/odds-delta"
  }
}
```
...
The `data` array contains the same odds objects as the [`/odds`](/api-reference/odds) endpoint. Only odds updated after the `since` timestamp are included.
...
- [Odds Snapshot](/api-reference/odds) - Get the full current odds snapshot
- [SSE Stream](/api-reference/stream) - Real-time push updates via Server-Sent Events
- [WebSocket Stream](/api-reference/websocket) - Real-time push updates via WebSocket
