...
**Live game state:** EV rows do not carry scores, period, or clock. Live game state is served exclusively by the [Game State](/api-reference/gamestate) endpoint and the `gamestate` stream channel. Join rows to game state by `event_id`.
</Callout>
...
| `market` | string | all | Filter by market type(s), comma-separated. Supports [category aliases](/api-reference/odds#market-category-aliases) (`main`, `spread`, `total`, `props`) or exact types (`point_spread`, `player_points`). |
...
- [Low Hold Opportunities](/api-reference/opportunities-low-hold) - Tightest lines across books
