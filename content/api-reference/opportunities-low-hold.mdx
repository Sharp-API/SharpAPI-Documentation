---
description: "GET /api/v1/opportunities/low_hold — Find markets with the lowest vig (hold) across sportsbooks for the tightest available lines. Ideal for sharp bettors seeking minimal juice."
---
...
**Breaking change:** The `game_state` field (score/period/clock) was removed from this response. Live game state is now served exclusively by the [Game State](/api-reference/gamestate) endpoint and the `gamestate` stream channel. Join rows to game state by `event_id`.
</Callout>
...
```json
{
  "error": {
    ...
    "docs": "https://docs.sharpapi.io/api-reference/overview"
  }
}
```
...
5. **Use streaming** — Subscribe to `low_hold:detected` events via [SSE](/api-reference/stream) or [WebSocket](/api-reference/websocket) for real-time alerts
