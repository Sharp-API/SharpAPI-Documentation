---
description: "GET /api/v1/opportunities/middles — Find middle opportunities where different sportsbook lines create a gap where both sides can win. Includes probability estimates and stake sizing."
---
...
| `market` | string | all | Filter by market type(s), comma-separated (e.g. `point_spread`, `total_points`). Supports category aliases — see [Odds: Market Category Aliases](/api-reference/odds#market-category-aliases). |
| `min_size` | number | 0.5 | Minimum middle size in points |
...
| `odds.probability` | number | Implied probability (0-1) |
| `odds.fair_probability` | number | Fair probability adjusted for overround |
