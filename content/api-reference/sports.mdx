...
data.forEach(sport => {
  console.log(`${sport.display_name}: ${sport.event_count} events (${sport.live_count} live)`);
...
for sport in result['data']:
    print(f"{sport['display_name']}: {sport['event_count']} events ({sport['live_count']} live)")
