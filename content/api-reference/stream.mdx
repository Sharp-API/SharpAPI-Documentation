...
| Field | Type | Description |
|-------|------|-------------|
| `odds` | array | Array of full `Odds` objects (see [Odds endpoint](/api-reference/odds) for all fields) |
...
| `odds_changed_at` | string | ISO 8601 timestamp of the sportsbook's own source update for this line, when available. On Pinnacle, carries forward while the underlying price/line/`is_live` flag are unchanged — see [Understanding Pinnacle's `odds_changed_at`](/concepts/pinnacle-odds-changed-at). |
...
- [+EV Opportunities](/api-reference/opportunities-ev) - REST endpoint for EV data (streamed via `ev:detected`)
- [Arbitrage Opportunities](/api-reference/opportunities-arbitrage) - REST endpoint for arbs (streamed via `arb:detected`)
- [Low Hold Opportunities](/api-reference/opportunities-low-hold) - REST endpoint for low hold (streamed via `low_hold:detected`)
- [Middles Summary](/api-reference/opportunities-middles-summary) - Aggregate middle stats for dashboard polling
- [WebSocket API](/api-reference/websocket) - Bidirectional alternative to SSE
