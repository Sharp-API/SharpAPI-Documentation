...
- [Odds Snapshot](/api-reference/odds) — `odds_changed_at` and `last_seen_at` fields on the REST odds response
- [Streaming](/api-reference/stream) — `odds_changed_at` on `odds:update` deltas
- [Live vs. Pre-Match](/concepts/live-vs-prematch) — publish cadence and book delivery mechanisms
- [EV Calculation](/concepts/ev-calculation) — why Pinnacle is the sharp reference
