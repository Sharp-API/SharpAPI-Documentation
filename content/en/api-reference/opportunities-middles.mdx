...
| `market` | string | all | Filter by market type(s), comma-separated (e.g. `point_spread`, `total_points`). Supports category aliases — see [Odds: Market Category Aliases](/en/api-reference/odds#market-category-aliases). |
...
5. **Combine with the summary endpoint** — Use [`/middles/summary`](/en/api-reference/opportunities-middles-summary) for dashboard overviews
6. **Act quickly** — Lines converge as books adjust, closing the middle window
7. **Use streaming** — Subscribe to `middles:detected` events via [SSE](/en/api-reference/stream) or [WebSocket](/en/api-reference/websocket) for real-time alerts

## Related Endpoints

- [Middles Summary](/en/api-reference/opportunities-middles-summary) — Aggregate statistics without the full list
- [+EV Opportunities](/en/api-reference/opportunities-ev) — Positive expected value bets
- [Arbitrage Opportunities](/en/api-reference/opportunities-arbitrage) — Guaranteed profit opportunities
- [SSE Stream](/en/api-reference/stream) — Real-time `middles:detected` and `middles:expired` events
- [Best Odds](/en/api-reference/odds-best) — Compare odds across all books
