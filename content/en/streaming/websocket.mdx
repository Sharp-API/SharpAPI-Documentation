...
<Callout type="info">
For full API documentation including all message types, schemas, and close codes, see the [WebSocket API Reference](/en/api-reference/websocket).
</Callout>
...
<Callout type="info">
The server keeps a 2-minute replay buffer. Pass `resume=true&from_seq=N` on reconnect to skip the full snapshot and replay only missed events. For longer outages, omit these params for a full snapshot. See [Reconnection with Replay](/en/api-reference/websocket#reconnection-with-replay) for details.
</Callout>
