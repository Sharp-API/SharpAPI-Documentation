---
description: "SDK oficial de Python para SharpAPI — instala sharpapi desde PyPI para obtener acceso tipado con modelos Pydantic, streaming SSE, soporte asíncrono y autocompletado completo del IDE de serie."
---
...
El paquete oficial `sharpapi` de Python proporciona acceso tipado a todos los endpoints de SharpAPI con modelos Pydantic, streaming SSE y autocompletado completo del IDE.
...
Con soporte opcional para pandas:
...
Streaming en tiempo real con patrones basados en handlers o en iteradores.

<Callout type="warning">
**Importante:** los eventos `odds:update` son **deltas** — solo contienen los odds que han cambiado. Tu cliente debe mantener el estado local y fusionar las actualizaciones sobre él. Tratar cada evento como una snapshot completa producirá datos incorrectos.
</Callout>

### Basado en handlers (patrón decorador)
...
Cada oportunidad incluye metadatos de obsolescencia (staleness):
...
La información del rate limit está disponible tras cada solicitud:
...
## Utilidades de conversión de odds
...
Todas las respuestas usan modelos Pydantic con anotaciones de tipo completas:
