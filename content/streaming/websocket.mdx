---
description: "WebSocket streaming guide for SharpAPI — connect to wss://ws.sharpapi.io for real-time odds updates with bidirectional communication. Covers setup, subscriptions, and error handling."
---
...
<Callout type="info">
The server keeps a 2-minute replay buffer. Pass `resume=true&from_seq=N` on reconnect to skip the full snapshot and replay only missed events. For longer outages, omit these params for a full snapshot. See [Reconnection with Replay](/api-reference/websocket#reconnection-with-replay) for details.
</Callout>
