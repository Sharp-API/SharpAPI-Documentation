...
data.forEach(league => {
  console.log(`${league.display_name} (${league.sport}): ${league.event_count} events`);
});
```
  </Tabs.Tab>
  <Tabs.Tab>
...
for league in result['data']:
    print(f"{league['display_name']} ({league['sport']}): {league['event_count']} events")
```
  </Tabs.Tab>
</Tabs>
...
<Callout type="info">
The `event_count` and `live_event_count` values are dynamic. Off-season leagues (e.g., MLB in winter, WNBA in fall) will show 0 events.
</Callout>
