...
data.forEach(sport => {
  console.log(`${sport.display_name}: ${sport.event_count} events (${sport.live_event_count} live)`);
...
for sport in result['data']:
    print(f"{sport['display_name']}: {sport['event_count']} events ({sport['live_event_count']} live)")
...
| `live_event_count` | integer | Events currently live/in-play |
...
<Callout type="info">
The `event_count` and `live_event_count` values are dynamic and change based on current schedules. Off-season sports will show 0 events.
</Callout>
