...
class ArbScanner:
    ...
    def start(self):
        url = f'https://sharpapi.io/v1/stream?min_profit={self.min_profit}'
    ...
    def _process_arb(self, opp):
        ...
        logger.info(f"💰 ARB FOUND: {opp['profitPercent']:.2f}%")
        logger.info(f"   Event: {opp['eventName']}")

        for leg in opp['legs']:
            logger.info(f"   → {leg['sportsbook']}: {leg['selection']} @ {leg['odds']} ({leg['stakePercent']:.1f}%)")
...
def process_arb(opp):
    ...
    print(f"💰 {opp['eventName']} - ${profit:.2f} profit")
    for bet in bets:
...
class DiscordAlerter:
    ...
    async def send_arb(self, opp: dict, stakes: list):
        async with aiohttp.ClientSession() as session:
            ...
            embed = discord.Embed(
                title=f"💰 {opp['profitPercent']:.2f}% Arbitrage",
